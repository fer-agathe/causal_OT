...
`r colorize("Group 1", colGpe1)` could be, for example, `r colorize("Black individuals", colGpe1)`, whereas `r colorize("Group 0", colGpe0)` could be individuals who are not Black.

Assume that the objective is to define, for each individual in `r colorize("Group 1", colGpe1)`, a counterfactual category that reflects the distributional characteristics of `r colorize("Group 0", colGpe0)`. That is, we want to know what would be the medical treatment of a `r colorize("Black individuals", colGpe1)` with a given treatment (e.g., "`r colorize("C=no treatment", colC)`) had they been `r colorize("non-Black", colGpe0)`.
...
Each category is assigned an arbitrary numeric value (e.g., $A = 1$, $B = 2$, $C = 3$), allowing us to define a cost matrix based on the absolute difference between encoded categories. That is, the cost of matching an individual from `r colorize("Group 1", colGpe1)` with category $x_{j1}$ to an individual from `r colorize("Group 0", colGpe0)` with category $x_{i0}$ is given by $C_{ij} = |x_{i0} - x_{j1}|$.

A linear sum assignment problem can then be used to find the matching that minimizes the total cost across pairs.

The matched category from `r colorize("Group 0", colGpe0)` is interpreted as the counterfactual category for the corresponding `r colorize("Group 1", colGpe1)` individual.
...
We compute the number of observations from `r colorize("Group 1", colGpe1)` matched with observations from `r colorize("Group 0", colGpe0)` per category.
```{r}
tb_coupling |> 
  group_by(x1, x0) |> 
  count() |> 
...
Then, we can visualize the results, using an alluvial plot (@fig-categ-alluv). All individuals in `r colorize("Group 1", colGpe1)` with label `r colorize("A", colA)` are matched directly to individuals in `r colorize("Group 0", colGpe0)` with the same label. The remaining 40% of individuals in `r colorize("Group 0", colGpe0)` labeled `r colorize("A", colA)` are then matched to `r colorize("Group 1", colGpe1)` individuals who originally had label `r colorize("B", colB)` or `r colorize("C", colC)`. This matching is performed probabilistically: among `r colorize("Group 1", colGpe1)` individuals with label `r colorize("B", colB)`, 60% retain their original label, and 40% are reclassified as `r colorize("A", colA)`; among those with label `r colorize("C", colC)`, 50% remain `r colorize("C", colC)`, and 50% are reclassified as `r colorize("A", colA)`.
