...
Let us follow the DAG from @fig-dag-compas and build the counterfactuals of non-white individuals: we thus transport individuals from $A=0$ to $A=1$, using the predictions on the test set. 
...
```{r fairadapt-rf-linear, message=FALSE, warning=FALSE}
...
We export the dataset to estimate the causal effect using normalizing flows with the python library medflow (see the [Github page of medflow](https://github.com/JesseZhou-1/medflow)). The python script is provided below but is not run during the compilation of this page. Note that this procedure does not provide individual effects, but aggregated ones.
```{r exporting-data-for-nf}
