# Gaussian Conditional Univariate Transport with Local Weights
...
## Sequential Optimal Transport {#sec-sot}
...
```{r show-transported-00}
c(round(T1x0,4), round(T2x0,4))
```


We define a wraper function, `transp_ot()`{.R} for the previous codes, to transport an observation from group 0 to group 1 with that framework.

```{r define-transp_ot}
#| code-fold: true
#| code-summary: The `transp_ot()`{.R} function.
...
#| fig-cap: Point $(x_{1,0} = 0, x_{2,0} =1)$ in <span style="color:#3fb3b2;">group 0</span> transported in <span style="color:#ffdd55;">group 1</span>. The <span style="color:gray;">intermediate point</span> from the sequential transport is shown in gray.
...
## Sequential Transport using Kernel Method
...
We define a function, `Transp2x0()`{.R}, to transport an observation under this Gaussian framework, using either a Gaussian or a Uniform (retangular) kernel.

```{r define-Transp2x0}
#| code-fold: false
#| code-summary: The `Transp2x0()`{.R} function. 
...
Let us now run some simulations. We will consider two different kernels: a Gaussian, and a Uniform. We will tranpsport two points: $(x_{1,0} = 0, x_{2,0} =1)$ and $(x_{1,0} = -2, x_{2,0} = -1)$. For each case, we generate 5,000 samples from the DGP shown in @sec-sot.
...
We can visualize the results. In @fig-gauss-weights we can visualize $x_{2|1,0}^\star$ (on the $y$-axis) when $(x_{1,0},x_{2,0})=(0,0)$ (on top) and when $(x_{1,0},x_{2,0})=(-2,-1)$ (below) using simulated Gaussian samples, for two choices of kernels (Gaussian on the left, uniform on the right). The horizontal dashed line indicates the transported value from standard optimal transport (as in @sec-sot).


```{r}
#| code-fold: true
#| code-summary: Codes to create the Figure
#| fig-cap: Conditional Optimal Transport using local weights. Value of $x_{2|1,0}^\star$ on the $y$-axis when $(x_{1,0}, x_{2,0}) = (0,0)$ (top) and $(-2, -1)$ (bottom), based on 5,000 simulated Gaussian samples. The left panel uses Gaussian kernels; the right panel uses rectangular (uniform) kernels. The horizontal dashed line indicates the transported value from standard optimal transport.
#| label: fig-gauss-weights
#| message: false
#| warnning: false
#| fig-height: 4.6
#| fig-width: 4.6
