...
Let us follow this DAGand build the counterfactuals of `r colorize("treated", colours[["1"]])`: we thus transport individuals from $A=1$ to $A=0$. Let us set a seed for reproducibility.
...
For a `r colorize("treated", colours[["1"]])` unit $i$, $Y_i(1)$ corresponds to the observed value and $Y_i(0)$ to its counterfactual, i.e. the transported value: $Y_i(0)\mid A=1 = x_{1,i}^\star$.

$$
\widehat{\text{ATT}}^{\text{COT}} = \frac{1}{n_1} \sum_{i:A_i=1}\bigl[ Y_i -  \hat{\mu}_0(x_{1,i}^\star)\bigr],
$$ {#eq-att-cot}
where $\hat{\mu}_0()$ is the predictive outcome model trained on the `r colorize("untreated", colours[["0"]])`, i.e., an estimation of $\mu_0(\boldsymbol{X}_i) = \mathbb{E}[Y_i \mid \boldsymbol{X}_i, A_i = 0]$


Let us split the data between `r colorize("treated", colours[["1"]])` and `r colorize("untreated", colours[["0"]])`
...
We use cross-fitting to estimate the average treatment on the `r colorize("treated", colours[["1"]])` using the AIPW estimator:
$$
...
We can have a look at the result of the first run to show, on a ternary plot (@fig-ternary-simul-1), the `r colorize("transported variable $X_3$", colours[["B"]])` from group 1 to group 0.

```{r}
#| code-fold: true
#| code-summary: Codes to create the Figure.
#| fig-cap: !expr glue::glue("$X_3$ values for individuals from <span style='color:{c0};'>group 0</span>, those from <span style='color:{c1};'>group 1</span>, and those <span style='color:{c2};'>transported</span> from group 1 to group 0 using sequential transport.", c0 = colours[["0"]], c1 = colours[["1"]], c2 = colours[["B"]])
...
The distribution of the estimated ATT on the 200 runs of the Monte-Carlo experiment are reported in @fig-simul-mc-att.


```{r}
#| code-fold: true
#| code-summary: Codes to create the Figure.
#| fig-cap: !expr glue::glue("Average treatment effect on the treated, {n} replications.", n = length(seeds))
#| label: fig-simul-mc-att
ggplot(
  data = ATT |> pivot_longer(cols = -seed) |> 
    mutate(
      name = factor(
        ...
        labels = c("True", "AIPW", "Causal Forests", "COT")
      )
    ),
  ...
  scale_fill_manual(
    ...
    values = c(
      ...
      "Causal Forests" = "#D55E00", "COT" = "#009E73"
    )
  ) +
  labs(y = NULL) +
  theme_paper() +
