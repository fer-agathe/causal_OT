...
tb_estim <- tb |> select(A, X1, X2, X3, Y)
tb_untreated <- tb_estim[ind_untreated, ]
tb_treated <- tb_estim[-ind_untreated, ]
```

Let us follow the DAG from @fig-dag and build the counterfactuals of Black individuals: we thus transport individuals from $A=0$ to $A=1$, using the predictions on the test set.
...
causal_effects_ot <- causal_effects_cf(
  data_untreated = tb_untreated, 
  data_treated = tb_treated,
  data_cf = as_tibble(tb_sinkhorn_transported), 
  Y_name = Y_name, 
  A_name = A_name, 
  A_untreated = A_untreated # 0
)
...
for (i in 1:length(seeds)) {
  ...
  # Optimal Transport
  tb_ot_transported <- optimal_transport_cf(tb,
                                            Y_name,
                                            A_name,
                                            A_untreated
                                            )
  ...
  tb_sinkhorn_transported <- optimal_transport_cf(tb,
                                                  Y_name,
                                                  A_name,
                                                  A_untreated,
                                                  pen = 0.1
...
save(res_simul_opt_trans, 
    res_simul_sink_trans, 
    res_simul_seq_trans, 
    res_simul_effects, file = "../output/res_simul.rda")
...
We can have a look at the `r colorize("transported values", colours[["transp"]])` for variable $X_3$ (from `r colorize("group 0", colours[["0"]])` to `r colorize("group 1", colours[["1"]])`) of the first run, on a ternary plot (@fig-ternary-simul-1).

```{r}
#| code-fold: true
#| code-summary: Codes to create the Figure.
#| fig-cap: !expr glue::glue("$X_3$ values for individuals from <span style='color:{c0};'>group 0</span>, those from <span style='color:{c1};'>group 1</span>, and those <span style='color:{c2};'>transported</span> from group 0 to group 1 using sequential transport.", c0 = colours[["0"]], c1 = colours[["1"]], c2 = colours[["transp"]])
...
ggtern(
  data = gen_data(i) |> select(A, X3_p_A,X3_p_B, X3_p_C) |> 
    ...
    bind_rows(
      as_tibble(res_simul_seq_trans[[i]]$transported_prob$X3) |> 
        rename_with(.fn = ~str_c("X3_p_", .)) |> 
        mutate(A = "0 to 1")
    ) |> 
    mutate(A = factor(A, levels = c("0", "1", "0 to 1"))),
  ...
  labs(x = "A", y = "B", z = "C") +
  scale_colour_manual(
    values = c("0" = colours[["0"]], "1" = colours[["1"]], "0 to 1" = colours[["transp"]])
  ) +
...
data_plot <- causal_effects |> 
  ...
  mutate(
    ...
    Method = factor(
      Method,
      levels = c(
        "causal_med", "seq_ot", "sink_ot", "ot"
      ),
      labels = c("Causal Med.", "Seq. OT", "Reg. OT", "OT")
    )
  )
ggplot(
  data = data_plot,
